...
- Import the file as raw source code (with some [Webpack](/docs/configuration-webpack--docs) configuration).
...
- [DefaultValue](/docs/components-defaultvalue--docs) - a table detailing a single prop's default value.
...
> #### Note:
>
> - Using these components for [User Interaction testing](https://storybook.js.org/docs/react/writing-tests/interaction-testing) may require waiting for render/update.

## Configuration

The default themes and component styles can be imported with the [default CSS styles](/docs/configuration-css--docs).

If are using Docs pages, you might also want to enable the light & dark mode theme functionality with the [DocsContainer](/docs/configuration-docscontainer--docs).

Optionally, some further configuration is required for some of the more advanced functionality:

- [Webpack](/docs/configuration-webpack--docs) - enables the "raw imports" and "unminified display of props" functionality.
